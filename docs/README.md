...
* Publish the [OpenPredict package to PyPI](https://pypi.org/project/trapi-predict-kit/) when a release is created (N.B.: the version of the package needs to be increased in [setup.py](https://github.com/MaastrichtU-IDS/translator-openpredict/blob/master/setup.py#L6) before).
...
Install `trapi-predict-kit` from the source code with `pip`, the package will be automatically updated when the files changes locally :arrows_counterclockwise:
...
Go to the `trapi-predict-kit` project:
